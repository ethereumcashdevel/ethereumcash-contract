...
// File: contracts/EthereumCashCoin.sol
 ...
 * @title coin contract
 */
contract EthereumCashCoin is ERC20Capped, ERC20Detailed, ERC20Burnable {
...
// File: contracts/EthereumCashCoinSafeMath.sol
 ...
 */
library EthereumCashCoinSafeMath {
...
// File: contracts/EthereumCashCoinMultiSigWallet.sol
...
/**
 * @title EthereumCashCoin Multi Signature contract
 */
contract EthereumCashCoinMultiSigWallet {
    /*  Enums  */
    ...
    /* Contract Created */
    event MultiSigCreated(address indexed creator, address indexed EthereumCashCoin);
    ...
    using EthereumCashCoinSafeMath for uint8;
    using SafeMath for uint256;
    ...
    EthereumCashCoin public EthereumCashCoin;
     ...
     * @param _EthereumCashCoinAddress EthereumCashCoin address
     *
     */
    constructor (address[] memory _signers, uint8 _requiredSign, address _EthereumCashCoinAddress, string memory _description) public {
        ...
        nextTx = 0;
        EthereumCashCoin = EthereumCashCoin(_EthereumCashCoinAddress);
        owner = msg.sender;
        description = _description;
        emit MultiSigCreated(owner, _EthereumCashCoinAddress);
    }
    ...
    /**
      ...
      * @param _amountUnit amount of EthereumCashCoin(Play part) to be transferred
      * @param _amountMinunit amount of EthereumCashCoin(Minunit part) to be transferred
      */
    ...
    // EthereumCashCoin.transfer will be fail with reason out of gas. Then the TX will remain
    ...
    function runTX(uint16 txid) internal senderIsSigner returns(bool) {
        ...
        if(request.txType == TxType.TRANSFER) {
            // transfer money
            EthereumCashCoin.transfer(request.who, request.amount);
            ...
            EthereumCashCoin.setAdmin(request.who);
        } else if(request.txType == TxType.OWNER_CHANGE) {
            EthereumCashCoin.setOwner(request.who);
        } else if(request.txType == TxType.VAULT_CHANGE) {
            EthereumCashCoin.setVault(request.who);
        } else {
            return false;
        }
        return true;
