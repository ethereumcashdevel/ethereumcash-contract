...
/**
 * EthereumCashCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("EthereumCashCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneEthereumCashCoinInMinunit, NoOfTokensInEthereumCash;
  let coin
  ...
  before(async () => {
    ...
    OneEthereumCashCoinInMinunit = BIG(await coin.getOneEthereumCashCoin());
    NoOfTokensInEthereumCash = NoOfTokensInMinunit.div(OneEthereumCashCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "EthereumCash");
    assert.equal(await coin.name(), "EthereumCashCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneEthereumCashCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneEthereumCashCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneEthereumCashCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfEthereumCash = OneEthereumCashCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfEthereumCash, {from: admin});
    assert.equal(await balanceOf(owner), halfEthereumCash.toString());
    assert.equal(await balanceOf(admin), halfEthereumCash.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInEthereumCash.sub(BIG(1)).mul(OneEthereumCashCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneEthereumCashCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneEthereumCashCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneEthereumCashCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneEthereumCashCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneEthereumCashCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneEthereumCashCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneEthereumCashCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 EthereumCash
    await coin.transfer(user1, OneEthereumCashCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 EthereumCash with 5 EthereumCash reserve
    await coin.setReserve(user1, OneEthereumCashCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 EthereumCash with 5 EthereumCash reserve
    await coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 EthereumCash with 5 EthereumCash reserve
    coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneEthereumCashCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneEthereumCashCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
