...
const EthereumCashCoinMultiSigFactory = artifacts.require('EthereumCashCoinMultiSigWalletWithMint.sol');
...
contract("EthereumCashCoinMultiSigWalletWithMint", (accounts) => {
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestMintInEthereumCash(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestMintInEthereumCash(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Add and remove a minter", async () => {
      ...
      assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OneEthereumCashCoinInMinunit, {from: user1}));
      let afterBalance = await coinInstance.balanceOf(user1);

      // balance difference should be same as the minted amount
      assert.equal(afterBalance.sub(prevBalance).toString(), OneEthereumCashCoinInMinunit.toString());
      ...
      assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OneEthereumCashCoinInMinunit, {from: user1}));
  ...
  it("test mint using multisig wallet", async () => {
        ...
        const thousandEthereumCash = BIG(1000)
        const zero = BIG(0)

        const MSG1 = "MINT 1000 EthereumCash to user1";
        const trSubmit = await walletInstance.requestMintInEthereumCash(user1, thousandEthereumCash, zero, MSG1, {from:vault});
        ...
        assert.equal(afterBalance.toString(), thousandEthereumCashInMinunit.toString());
