...
const EthereumCashCoin = artifacts.require('EthereumCashCoin.sol');
const EthereumCashCoinMultiSigFactory = artifacts.require('EthereumCashCoinMultiSigWallet.sol');
const web3 = EthereumCashCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return EthereumCashCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployEthereumCashCoin = (owner,admin,vault) => {
  return EthereumCashCoin.new("EthereumCash", "EthereumCashCoin", owner, admin, vault);
};
...
contract("EthereumCashCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneEthereumCashCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployEthereumCashCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneEthereumCashCoinInMinunit = await coinInstance.getOneEthereumCashCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.EthereumCashCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandEthereumCashInMinunit = OneEthereumCashCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandEthereumCashInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandEthereumCashInMinunit.toString());

    // Add EthereumCashcoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneEthereumCashCoin * 1);
    //

    const MSG1 = "send 1EthereumCash and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInEthereumCash(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneEthereumCashCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInEthereumCash(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInEthereumCash(user1, 1, 0, "test addsigner-1", {from: user1}));
